...
import { connectDB, Settings, Page, Stat, Celebrity, WhyChoose, EventType } from './db';

export async function getCommonProps() {
  try {
    await connectDB();
    const [settingsRaw, footerPages, stats, celebrities, whyChoose, eventTypes] = await Promise.all([
      Settings.find({}).lean(),
      ...
      Celebrity.find({ visible: true }).sort({ order: 1 }).lean(),
      WhyChoose.find({ visible: true }).sort({ order: 1 }).lean(),
      EventType.find({ visible: true }).sort({ order: 1 }).lean(),
    ]);
    const site = {};
    const keyMap = {
    ...
    return { site: {}, footerPages: [], stats: [], celebrities: [], whyChoose: [], eventTypes: [] };
  }
}
